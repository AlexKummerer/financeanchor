...
INSERT INTO `__new_recurring_items`("id", "user_id", "created_at", "updated_at", "name", "amount_cents", "interval_months", "start_month", "due_day", "kind", "category_id", "reserve_pot_id", "account_id") SELECT "id", "user_id", "created_at", "updated_at", "name", "amount_cents", "interval_months", "start_month", "due_day", "kind", "category_id", "reserve_pot_id", NULL FROM `recurring_items`;--> statement-breakpoint
...
ALTER TABLE `transactions` ADD `import_label` text;--> statement-breakpoint
CREATE INDEX `transactions_import_label_idx` ON `transactions` (`user_id`,`import_label`);