...
- Kredit-Plan: nur tatsächliche Zahlungen (Rate, Frist-Rate bzw. Zurücklegen, eigene `extraMonthlyCents`); Ziele werden nur geprüft. „Verfügbar für Kredite“ (`loanBudgetCents`) dient ausschließlich `loanAdvice` und ändert keinen Plan: fehlt Geld, nur Aufstellung des Geplanten; ist Geld übrig, Vorschläge „meiste Zinsersparnis“ und „schnellste Entlastung“ (keine Strategie-Auswahl mehr), Ziel-Vorschläge nur, wenn sie passen; ebenso „Übernehmen“ an der Ziel-Lücke der Kreditkarte. Geplant wird für `LoanPlanner.adviceMonth` (nächster Monat, sobald der laufende bei den Krediten ganz gebucht ist); Extra-Tilgungen gelten ab `extraFromMonth`.
- Anmelden mit Google/Microsoft/Apple: `apps/backend/src/social.ts`; ein Anbieter ist aktiv, wenn seine Secrets gesetzt sind (`/api/auth-options`). Immer `disableSignUp` und `disableImplicitLinking` – Anbieter werden nur in den Einstellungen mit einem bestehenden Konto verbunden. Externe Weiterleitungen über `core/platform/external-navigation.ts`.
- Kreditkarten (`docs/plan-kreditkarten.md`): Konto-Art `credit_card`, Stand wird aus Buchungen berechnet (`cardBalance`, Backend `services/cards.ts`) – `balance_cents` ist nur der Startstand. Abbuchung ist `card_payment` und zählt nirgends als Ausgabe. Update-Schemas immer mit `patchSchema(...)`, nie `.partial()` auf Felder mit Standardwert.
- CSV-Import (`docs/plan-csv-import.md`): Lesen/Erkennen in `packages/shared/src/import` (reines TS, Bank-Presets in `presets.ts`), Datei wird nur im Browser gelesen. Doppelte verhindert `transactions.import_key` (eindeutig je Nutzer). Nichts wird automatisch übernommen.
